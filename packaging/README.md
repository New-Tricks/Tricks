...
| Homebrew | Tap [`new-tricks/homebrew-tap`](https://github.com/new-tricks/homebrew-tap), formula `Formula/newtricks.rb` (installs the `tricks` binary; source copy in `packaging/homebrew/`). `brew install new-tricks/tap/newtricks`. The release workflow renders `packaging/homebrew/newtricks.rb` with the tag's source tarball `url`/`sha256` (`render.sh`), runs `brew style`, and pushes it to the tap using the `HOMEBREW_TAP_DEPLOY_KEY` secret (a write deploy key on the tap). Edit the template here, never the tap directly. Move to homebrew-core later. `upgrade` defers to `brew upgrade`. |
