# New Tricks v0.5.0 — implementation notes
...
| Links and trials | `tests/links.rs` (scenarios 1, 5 + `try` into the current project, `list --trials` here / `--all`, `untry` of one trial and of the project's, repo skills linked globally in dev mode and unlinked together, `list --links` needing `--all` outside a repo, `list` showing where skills are linked, `link`/`try`/`unlink`/`untry` pointing at each other for the wrong kind of skill, worktree exclude sharing, starred trust) | 6 pass |
...
| Discovery | `tests/discover.rs`: `info` (metadata and frontmatter, no body) and `view` (the file as is, supporting files, JSON); a new user config lists the recommended catalogs, `catalog remove` removes, `catalog add --recommended` restores, an existing config is never rewritten | 2 pass |
...
35. **`link`/`unlink` and `try`/`untry` are separate sets** (0.5): `link` takes source repo skills (default: the user-level agent directories) and `unlink` removes them (this repo's; `--all` for every source repo, required outside one); `try` takes anything else — upstream skills and local folders outside the repo (default: the current project) — and `untry` removes trials (a named one wherever it is, else this project's; `--global`/`--to`/`--all`). `list --links` shows this repo's links (`--all` outside a repo), `list --trials` the trials here and at user level (`--all`: everywhere). Each command points to the other when given the wrong kind. Repo skill links (any scope) follow `edit`, `use`, `merge` and `sync`; trials are fixed revisions, never locked or updated.
...
39. **Experiments are always on a branch** (0.5): `edit <skill>` uses `-b`, else the experiment under way, else `draft/<skill>`; there is no main-checkout mode, and `edit` no longer vendors upstream skills (use `vendor`). Drafts are committed with `edit --commit -m`, with a `Tricks-Agent:` trailer when an agent is detected. `edit --done` stops editing without merging. Sync snapshots are released by the next `tricks` command after the sync is committed with git (`list`, `link`, …; the extension's refresh does it too).
...
45. **`view` prints the file as is** (0.5; 0.4 rendered Markdown with `termimad`); pipe it to a renderer such as `glow -`. `--json` gives `{skill, path, content}`.
...
49. **Worktrees live in the source repo** at `.tricks/work/<branch>/` (0.5; was the data directory), with `/.tricks/` added to `.gitignore` by `init` or the first `edit`. Repo discovery maps a worktree's own `tricks.toml` back to its source repo, so commands run in the draft (e.g. from `edit --shell`) act on the repo. `edit` prints only the draft's path on stdout (`cd "$(tricks edit <skill>)"`); `--shell` runs `$SHELL` (`%COMSPEC%` on Windows) there with `TRICKS_EDITING=<skill>@<branch>`.
