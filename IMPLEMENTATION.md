...
| Extension | `extension/out/test/runTest.js` in VS Code 1.128.1 (throwaway profile): activation, commands, status via real binary, lint diagnostics, virtual documents, Markdown preview; Discover messages through the real core (search, copy, `tricks.search` handing the view its query); publishing from the pre-flight panel through the core's confirmation into a local target repo (files, tag) | pass |
